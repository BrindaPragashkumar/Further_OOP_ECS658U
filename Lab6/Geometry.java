...
public class Geometry {
    ...
    private static boolean oneWayOverlapCheck(ArrayList<Vec2d> polygonA,
                                              ArrayList<Vec2d> polygonB) {
        for (int i = 0; i < polygonA.size(); i++) {
            Vec2d edge = polygonA.get(i).subtract(polygonA.get((i + 1) %
                    polygonA.size()));
            Vec2d axis = edge.prep();
    ...
    public static boolean polygonsOverlap(ArrayList<Vec2d> polygonA,
                                          ArrayList<Vec2d> polygonB) {
        return oneWayOverlapCheck(polygonA, polygonB) &&
                oneWayOverlapCheck(polygonB, polygonA);
    }
    ...
    public static boolean contains(ArrayList<Vec2d> polygon, ArrayList<Vec2d>
            other) {
        for (Vec2d p : other) {
